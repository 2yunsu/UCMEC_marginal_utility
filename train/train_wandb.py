...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            # scenario_name을 전달
            env = DiscreteActionEnv(scenario_name=all_args.scenario_name)
...
def main(args):
    ...
    # 환경을 먼저 생성하여 N, M 값을 가져옴
    from envs.env_discrete import DiscreteActionEnv
    temp_env = DiscreteActionEnv(all_args.scenario_name)
    
    # N, M 값 추출 (환경이 이 속성을 가진 경우)
    env_params = ""
    if hasattr(temp_env.env, 'N') and hasattr(temp_env.env, 'M'):
        env_params = f"_N{temp_env.env.N}_M{temp_env.env.M}"

    # wandb 초기화
    if all_args.use_wandb:
        wandb.init(
            project="UCMEC-mmWave-Fronthaul",
            name=f"{all_args.experiment_name}_seed{all_args.seed}",
            config=vars(all_args),
            tags=[all_args.algorithm_name, all_args.env_name, all_args.scenario_name],
            group=all_args.experiment_name,
            notes=f"Training {all_args.algorithm_name} on {all_args.scenario_name}"
        )
